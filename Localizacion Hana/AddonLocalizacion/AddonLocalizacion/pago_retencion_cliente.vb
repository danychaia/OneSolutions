...
Public Class pago_retencion_cliente
        ...
        Try
                        ...
                        If (pVal.ItemUID = "Item_3") Then
                            Try
                                ...
                                Try
                                    ...
                                    oRecordB.DoQuery("CALL INF_PAGO_RETENCION ('3','" & txtCliente.Value & "','" & val & "',0,0,0,0,'','','')")
                                    ...
                                    total.DoQuery("CALL INF_PAGO_RETENCION ('4','" & txtCliente.Value & "','" & val & "',0,0,0,0,'','','')")
                                    ...
                                    GC.Collect()
                                    total = oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                                    total.DoQuery("CALL INFORMACION_PAGO ('2','','','','','" & txtCliente.Value & "','" & val & "')")
                                ...
                                Try
                                    ...
                                    'UDT_UF.FilterCFL(oForm, "CFL_1", "DocEntry", val)
                                    Dim sql As String = "CALL INF_PARTNER_OPE (2,'" & txtRuc.Value & "','','','','','')"
                                    Try
            ...
            If pVal.Before_Action = True And pVal.FormUID = "pCliente" And pVal.ItemUID = "Item_21" Then
                Try
                    ...
                    oRecordv.DoQuery("CALL INF_PAGO_CLIENTE ('1','" & txtDocumento.Value & "','" & txtCliente.Value & "')")
                    ...
                    'Return
                    ' End If
                    oProgressBar = SBOApplication.StatusBar.CreateProgressBar("Generando Compras", 2, True)
                    ...
                    InPay.Remarks = "Pago Retencion para cliente " & txtCliente.Value.Trim
                    ...
                    Dim valor As String
                    Dim sqls = "CALL INF_PAGO_RETENCION ('5','" & txtCliente.Value & "','" & txtDocumento.Value & "',0,0,0,0,'','','')"
		    oProgressBar.Value = 1
                    ...
                    While orecordPago.EoF = False
                        If Double.Parse(orecordPago.Fields.Item("U_T_BASE").Value) > 0 Then
                            ...
                            InPay.CreditCards.CardValidUntil = DateTime.ParseExact(txtCaducidad.Value, "yyyyMMdd", CultureInfo.InvariantCulture)
                            InPay.CreditCards.CreditCardNumber = txtAutorizacion.Value.Trim   ' Just need 4 last digits
                            ...
                            InPay.CreditCards.VoucherNum = txtNumeroReten.Value.Trim    ' Need to give the Credit Card confirmation number.
                            Dim orecord As SAPbobsCOM.Recordset
                            orecord = oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                            orecord.DoQuery("CALL SP_CUENTA_RETENCIONES ('Pago','" & orecordPago.Fields.Item("U_CUENTAB").Value & "')")
                    ...
                    Else
                        ...
                        oRecordv = oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                        oRecordv.DoQuery("CALL INF_PAGO_CLIENTE ('2','" & txtDocumento.Value & "','" & txtCliente.Value & "')")
                        System.Runtime.InteropServices.Marshal.ReleaseComObject(oRecordv)
                        oRecordv = Nothing
                        GC.Collect()
                        oRecordv = oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                        oRecordv.DoQuery(" CALL INFORMACION_PAGO ('1','" & txtAutorizacion.Value & "','" & txtNumeroReten.Value & "','" & txtFechaReten.Value & "','" & txtCaducidad.Value & "','" & txtCliente.Value & "','" & txtDocumento.Value & "')")
                        System.Runtime.InteropServices.Marshal.ReleaseComObject(oRecordv)
                        oRecordv = Nothing
                        GC.Collect()
    ...
    Private Sub visualizardata(p1 As String, p2 As String)
        Try
            ...
            Dim sql As String = "CALL BUSCAR_INFO_RETENCION ('" & p1 & "','" & p2 & "')"
    ...
    Private Function obtenerCliente(val As String) As String
        ...
        Try
            Dim orecord As SAPbobsCOM.Recordset
            orecord = oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            Dim sql As String = "SELECT A." & Chr(34) & "CardCode" & Chr(34) & " FROM OINV A WHERE A." & Chr(34) & "DocEntry" & Chr(34) & " = '" & val & "'"
            orecord.DoQuery(sql)
            If orecord.RecordCount > 0 Then
                Return orecord.Fields.Item(0).Value
            End If
    ...
    Private Function obtenerImpuesto(val As String) As String
        ...
        Try
            Dim orecord As SAPbobsCOM.Recordset
            orecord = oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            Dim sql As String = "SELECT SUM(A." & Chr(34) & "VatSum" & Chr(34) & ") FROM INV1 A WHERE A." & Chr(34) & "DocEntry" & Chr(34) & " = '" & val & "'"
            orecord.DoQuery(sql)
            If orecord.RecordCount > 0 Then
                Return orecord.Fields.Item(0).Value
            End If
    ...
    Private Sub cargarRetenciones(p1 As String, p2 As String)
        Try
            ...
            Dim sql As String = "CALL INF_PAGO_RETENCION ('2','" & p1 & "','" & p2 & "',0,0,0,0,'','','')"
            oForm.DataSources.DataTables.Item(0).ExecuteQuery(sql)
            gridView.DataTable = oForm.DataSources.DataTables.Item("MyDataTable")
            gridView.AutoResizeColumns()
            gridView.Columns.Item(0).Visible = False
    ...
    Private Sub cargarcombo()
        Try
            ...
            oRecord = oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            oRecord.DoQuery("CALL SP_CUENTA_RETENCIONES ('Fuente','')")
            If oRecord.RecordCount > 0 Then
            ...
            oRecord = oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            oRecord.DoQuery(" CALL SP_CUENTA_RETENCIONES ('IVA','')")
